...
(async () => {
    ...
    for (c_key in country) {
        for (a_key in appType) {
            ...
            await getIosPage(page, request_url, p_key, country[c_key]);
        }
    }
    // 关闭浏览器
    await browser.close();
...
async function login(page){
    const mobile = '15559079670';
    const password = 'xy520.';
    // 跳转登录界面
    await page.goto('https://app.diandian.com/login', {waitUntil: 'networkidle2'});
    // 切换登录方式
...
// 异步前往IOS游戏列表
async function getIosPage(page, url, platform, c_id) {
...
async function getIos(page, url) {
    ...
    if (screen_list.length != 0) {
        ios_return.img_list = getShortUrl(screen_list, "-ssl.mzstatic.com/image/thumb/")
    } else {
        ios_return.img_list = ''
    }
    ...
    ios_return.bundle_id = base_list[1];
    ios_return.country = base_list[6];
...
async function getEstimateDownload(page, url) {
    ...
    let download_num = StrToNumber(download_list[1] == '¥ 500' ? '0' : download_list[1]);
